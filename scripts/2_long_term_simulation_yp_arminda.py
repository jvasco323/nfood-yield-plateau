...
# 1. Loop per weather station
    ...
    for station in weather_stations:
        ...
        for CO2, yr in zip(CO2levels, years):
            ...
            # 4. Load agromanager
            agrod = yaml.safe_load(open(os.path.join(input_dir, "agro-arminda", f"Agrom_WW_Monocrop_{yr}.txt")))
        ...
        path = os.path.join(output_dir, f"arminda-1977/yp-long-term-daily-{station}-co2-{soil}.xlsx")
        ...
        path = os.path.join(output_dir, f"arminda-1977/yp-long-term-summary-{station}-co2-{soil}.xlsx")
        book = load_workbook(path)
        writer = pd.ExcelWriter(path, engine='openpyxl')
        writer.book = book
        writer.sheets = dict((ws.title, ws) for ws in book.worksheets)
