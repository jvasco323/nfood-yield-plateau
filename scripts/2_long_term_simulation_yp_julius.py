...
# 1. Loop per weather station
    ...
    for station in weather_stations:
        ...
        path = os.path.join(output_dir, f"julius-2009/yp-long-term-daily-{station}-co2-{soil}.xlsx")
        ...
        path = os.path.join(output_dir, f"julius-2009/yp-long-term-summary-{station}-co2-{soil}.xlsx")
        book = load_workbook(path)
        writer = pd.ExcelWriter(path, engine='openpyxl')
        writer.book = book
        writer.sheets = dict((ws.title, ws) for ws in book.worksheets)
